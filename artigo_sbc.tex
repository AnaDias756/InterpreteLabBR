...
\cite{bhaskaran2025} investigaram a aplicação de OCR para avaliação
automatizada de transcrições acadêmicas, demonstrando que a combinação de
técnicas de extração com estratégias de validação pós-OCR aumenta
significativamente a precisão dos dados estruturados obtidos. Essa abordagem
é análoga à adotada no InterpreteLabBR, que combina \textit{parsing} de texto
com OCR como estratégia de \textit{fallback}, e aplica regras de validação
para verificar se os valores extraídos estão dentro de faixas plausíveis
antes de submetê-los ao motor de classificação. A questão do
